<script>
    ...
    const createQuestion = async () => {
        const data = await createOrUpdateQuestionFn({title ,status ,shouldBe ,typeId , turn ,surveyId , isUsedOk});
        if(data.status == true){
            ...
            <div style="margin: auto;" class="column is-10-desktop is-offset-2-desktop is-9-tablet is-offset-3-tablet is-12-mobile">
                <div class="box">
                                ...
                                <div class="column">
                                    <div class="">
                                        <label class="checkbox">
                                            <input type="radio" name="typeId"  value={type.id} on:click={e => changeTypeId(e)}>
                                            {type.title}
                                        </label>
                                    </div>
                                </div>
                        ...
                        <div class="box columns mt-3 mb-5 is-multiline is-tablet back-eee">
                            {#each surveys as survey}
                                <div class="column">
                                    <div class="">
                                        <label class="checkbox">
                                            <input type="radio" name="surveyId"  value={survey.id} on:click={e => changeSurveyId(e)}>
                                            {survey.title}
                                        </label>
                                    </div>
                                </div>
                    ...
                    <div class="field is-grouped submit-parent" >
                            <button on:click={createQuestion} class="button is-link">ذخیره</button>
                    </div>
                </div>
            </div>
        </div>
