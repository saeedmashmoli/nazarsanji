<script>
    ...
    import { createOrUpdateTemplateFn, getTemplateFn ,getParametersForCreateOrUpdateTemplateFn} from '../../Api/templateApi';
    ...
    onMount( async() => {
        ...
        const result = await getTemplateFn(id);
    ...
    const updateTemplate = async () => {
        ...
        isLoading = true;
        const data = await createOrUpdateTemplateFn({ title, tempNumber : parseInt(tempNumber) ,parameters : value , link , status } , id);
        if(data.status == true){
    ...
    const changeParameters =  (parameterId) => {
        ...
        if(parameters.includes(parameterId)){
            ...
            parameters = [...parameters , parameterId]
        }
    }
</script>
<svelte:head>
    ...
    {:else}
        <div class="p-2">
            <div class="columns is-variable is-desktop">
                <div class="column">
                    <h3 class="title text-center is-size-4">ویرایش قالب</h3>
                </div>
            </div>
            <div style="margin: auto;" class="column is-10-desktop is-offset-2-desktop is-9-tablet is-offset-3-tablet is-12-mobile">
                <div class="box">
                                ...
                                <div class="column is-one-quarter">
                                    <div class="">
                                        <label class="checkbox">
                                            <input type="checkbox" name="parameter" checked={parameters.includes(parameter.id)}  value={parameter.id} on:click={e => changeParameters(parameter.id)}>
                                            {parameter.label}
                                        </label>
                                    </div>
                                </div>
                    ...
                    <div class="field is-grouped submit-parent" >
                        <button on:click={updateTemplate} class="button is-link" class:is-loading={isLoading}>ذخیره</button>
                    </div>
                </div>
            </div>
        </div>
